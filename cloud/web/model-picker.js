...
(() => {
  ...
  row.innerHTML='<label>模型 <select id="bridge-model" aria-label="模型"></select></label><label>强度 <select id="bridge-effort" aria-label="推理强度"></select></label><span id="model-running" role="status"></span>';
  composer.before(row);
  ...
  function paint(){
    const choice=drafts[currentId]||choices[currentId],manual=Boolean(choice?.model),mv=manual?choice.model:defaults.model;
  ...
  function draft(m,e){drafts[currentId]={model:m,effort:e};error='';paint();}
  model.onchange=()=>draft(model.value,defaultEffort(model.value));
  effort.onchange=()=>draft(model.value,effort.value);
  ...
  window.modelPicker={
    ...
    capture(id,isNew=false){
      ...
      return {model:m,effort:e,followDefaults:isNew?!newManual:!(drafts[id]||choices[id])?.model};
    },
    accepted(id,settings){
      if(id&&!settings.followDefaults){
        choices[id]={model:settings.model,effort:settings.effort,updated:Date.now()/1000};
        const d=drafts[id];
        if(d?.model===settings.model&&d?.effort===settings.effort)delete drafts[id];
      }
