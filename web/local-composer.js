...
function refreshLocalNewForm(){const kind=localThreadKind();localExistingRow.hidden=kind!=='existing';localProjectRow.hidden=kind!=='project';const cwd=kind==='standalone'?localBridgeConfig.standaloneDir:kind==='project'?`${localBridgeConfig.projectsRoot.replace(/[\\/]+$/,'')}\\${localNewProjectName.value.trim()||'项目名称'}`:localNewProject.value;localNewLocation.textContent=`本机目录：${cwd||'请先选择项目'}`;}
